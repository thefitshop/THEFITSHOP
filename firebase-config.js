...
try {
  if (typeof firebase !== 'undefined' && firebaseConfig.apiKey !== 'YOUR_API_KEY') {
    ...
    _fbReady = true;
    console.log('[FitLab] Firebase initialised OK');
  } else {
    console.warn('[FitLab] Firebase SDK not loaded — typeof firebase:', typeof firebase);
  }
} catch (e) {
  console.warn('[FitLab] Firebase init failed:', e.message);
}

// ── Save a single order to Firestore ─────────
function saveOrderToCloud(order) {
  if (!_fbReady) return Promise.resolve();
  return db.collection('fitlab_orders').doc(order.id).set(order)
...
function saveStatusToCloud(orderId, status) {
  if (!_fbReady) return Promise.resolve();
  return db.collection('fitlab_order_statuses').doc(orderId).set({ status: status })
...
function saveStockToCloud(stock) {
  if (!_fbReady) return Promise.resolve();
  return db.collection('fitlab_settings').doc('stock').set(stock)
...
function saveCostsToCloud(costs) {
  if (!_fbReady) return Promise.resolve();
  return db.collection('fitlab_settings').doc('costs').set(costs)
...
function syncOrdersFromCloud() {
  if (!_fbReady) return Promise.resolve([]);
  return db.collection('fitlab_orders').get().then(function (snap) {
    var orders = [];
    ...
    return JSON.parse(localStorage.getItem('fitlab_orders')) || [];
  });
}
...
function syncStatusesFromCloud() {
  if (!_fbReady) return Promise.resolve({});
  return db.collection('fitlab_order_statuses').get().then(function (snap) {
    var statuses = {};
    snap.forEach(function (doc) { statuses[doc.id] = doc.data().status; });
    localStorage.setItem('fitlab_order_statuses', JSON.stringify(statuses));
    return statuses;
  }).catch(function (e) {
    console.error('[Firebase] Sync statuses failed:', e);
    return JSON.parse(localStorage.getItem('fitlab_order_statuses')) || {};
  });
}
...
function syncStockFromCloud() {
  if (!_fbReady) return Promise.resolve({});
  return db.collection('fitlab_settings').doc('stock').get().then(function (doc) {
    if (doc.exists) {
      localStorage.setItem('fitlab_stock', JSON.stringify(doc.data()));
      return doc.data();
    }
    return JSON.parse(localStorage.getItem('fitlab_stock')) || {};
  }).catch(function (e) {
...
function syncCostsFromCloud() {
  if (!_fbReady) return Promise.resolve({});
  return db.collection('fitlab_settings').doc('costs').get().then(function (doc) {
    if (doc.exists) {
      localStorage.setItem('fitlab_costs', JSON.stringify(doc.data()));
      return doc.data();
    }
    return JSON.parse(localStorage.getItem('fitlab_costs')) || {};
  }).catch(function (e) {
...
function listenOrdersRealtime(onUpdate) {
  ...
  _ordersUnsub = db.collection('fitlab_orders').onSnapshot(function (snap) {
    var orders = [];
    snap.forEach(function (doc) { orders.push(doc.data()); });
    orders.sort(function (a, b) { return new Date(a.date) - new Date(b.date); });
    localStorage.setItem('fitlab_orders', JSON.stringify(orders));
...
function listenStatusesRealtime(onUpdate) {
  ...
  _statusesUnsub = db.collection('fitlab_order_statuses').onSnapshot(function (snap) {
    var statuses = {};
    snap.forEach(function (doc) { statuses[doc.id] = doc.data().status; });
    localStorage.setItem('fitlab_order_statuses', JSON.stringify(statuses));
...
function listenStockRealtime(onUpdate) {
  ...
  _stockUnsub = db.collection('fitlab_settings').doc('stock').onSnapshot(function (doc) {
    if (doc.exists) {
      localStorage.setItem('fitlab_stock', JSON.stringify(doc.data()));
...
function saveTrackingToCloud(orderId, trackingNumber) {
  if (!_fbReady) return Promise.resolve();
  return db.collection('fitlab_order_tracking').doc(orderId).set({
...
function syncTrackingFromCloud() {
  if (!_fbReady) return Promise.resolve({});
  return db.collection('fitlab_order_tracking').get().then(function (snap) {
    var tracking = {};
    snap.forEach(function (doc) {
      tracking[doc.id] = doc.data().trackingNumber || '';
    });
    localStorage.setItem('fitlab_order_tracking', JSON.stringify(tracking));
    return tracking;
  }).catch(function (e) {
    console.error('[Firebase] Sync tracking failed:', e);
    return JSON.parse(localStorage.getItem('fitlab_order_tracking')) || {};
  });
}
...
function listenTrackingRealtime(onUpdate) {
  ...
  _trackingUnsub = db.collection('fitlab_order_tracking').onSnapshot(function (snap) {
    var tracking = {};
    snap.forEach(function (doc) {
      tracking[doc.id] = doc.data().trackingNumber || '';
    });
...
function getTrackingFromCloud(orderId) {
  ...
  return db.collection('fitlab_order_tracking').doc(orderId).get().then(function (doc) {
...
function saveProductToCloud(product) {
  if (!_fbReady) return Promise.resolve();
  return db.collection('fitlab_products').doc(String(product.id)).set(product)
...
function syncProductsFromCloud() {
  if (!_fbReady) {
    console.warn('[FitLab] Firestore fetch skipped — Firebase not ready (_fbReady=false)');
    return Promise.resolve({});
  }
  console.log('[FitLab] Fetching products from Firestore...');
  return db.collection('fitlab_products').get().then(function (snap) {
    var products = {};
    snap.forEach(function (doc) { products[doc.id] = doc.data(); });
    console.log('[FitLab] Firestore fetch OK —', Object.keys(products).length, 'products loaded');
    return products;
  }).catch(function (e) {
    console.error('[FitLab] Firestore fetch failed:', e);
    return {};
  });
}
...
function getRoyalMailStatusFromCloud(orderId) {
  if (!_fbReady) return Promise.resolve(null);
  return db.collection('fitlab_order_royal_mail').doc(orderId).get().then(function (doc) {
    if (doc.exists) return doc.data();
    return null;
  }).catch(function (e) {
...
function syncRoyalMailFromCloud() {
  if (!_fbReady) return Promise.resolve({});
  return db.collection('fitlab_order_royal_mail').get().then(function (snap) {
    var rmStatuses = {};
    snap.forEach(function (doc) {
      rmStatuses[doc.id] = doc.data();
    });
    localStorage.setItem('fitlab_order_royal_mail', JSON.stringify(rmStatuses));
    return rmStatuses;
  }).catch(function (e) {
    console.error('[Firebase] Sync RM statuses failed:', e);
    return JSON.parse(localStorage.getItem('fitlab_order_royal_mail')) || {};
  });
}
